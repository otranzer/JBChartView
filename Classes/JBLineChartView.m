...
// Added
- (NSUInteger)lineChartDotsView:(JBLineChartDotsView *)lineChartDotsView horizontalRealIndex:(NSUInteger)horizontalIndex atLineIndex:(NSUInteger)lineIndex;
        ...
        for (NSUInteger lineIndex=0; lineIndex<numberOfLines; lineIndex++)
        {
            ...
            for (NSUInteger horizontalIndex=0; horizontalIndex<dataCount; horizontalIndex++)
            {
                ...
                if (![self.dataSource respondsToSelector:@selector(lineChartView:ignorePointForLine:atHorizontalIndex:)] ||
                    ![self.dataSource lineChartView:self ignorePointForLine:lineIndex atHorizontalIndex:horizontalIndex]) {
...
// Added
- (NSUInteger)horizontalRealIndexForLineIndex:(NSUInteger)lineIndex andHorizontalIndex:(NSUInteger)horizontalIndex
{
    if ([self.dataSource respondsToSelector:@selector(lineChartView:ignorePointForLine:atHorizontalIndex:)]) {
        int counter = -1;
        for (int i = 0 ; i < [self.dataSource lineChartView:self numberOfVerticalValuesAtLineIndex:lineIndex] ; i++) {
            
            if (![self.dataSource lineChartView:self ignorePointForLine:lineIndex atHorizontalIndex:i]) {
                counter++;
            }
            
            if (horizontalIndex == counter) return i;
        }
...
- (NSUInteger)lineChartDotsView:(JBLineChartDotsView *)lineChartDotsView horizontalRealIndex:(NSUInteger)horizontalIndex atLineIndex:(NSUInteger)lineIndex
{
    return [self horizontalRealIndexForLineIndex:lineIndex andHorizontalIndex:horizontalIndex];
}
...
- (NSInteger)lineIndexForPoint:(CGPoint)point
{
    ...
    for (NSUInteger lineIndex=0; lineIndex<numberOfLines; lineIndex++)
    {
        ...
        if ([self.dataSource lineChartView:self numberOfVerticalValuesAtLineIndex:lineIndex] > rightHorizontalIndex)
        {
            ...
            // Left point
            NSInteger realLeftIndex = leftHorizontalIndex;
            ...
            // Right point
            NSInteger realRightIndex = rightHorizontalIndex;
...
- (void)reloadData
{
    ...
    for (NSArray *lineData in chartData)
    {
        ...
        if ([self.delegate lineChartDotsView:self showsDotsForLineAtLineIndex:lineIndex]) // line at index contains dots
        {
            ...
            {
                NSInteger realIndex = [self.delegate lineChartDotsView:self horizontalRealIndex:horizontalIndex atLineIndex:lineIndex];
                ...
                UIView *currentDotView = [self.delegate lineChartDotsView:self dotViewAtHorizontalIndex:realIndex atLineIndex:lineIndex];
                ...
                if (currentDotView == nil)
                {
                    ...
                    CGFloat dotRadius = [self.delegate lineChartDotsView:self dotRadiusForLineAtHorizontalIndex:realIndex atLineIndex:lineIndex];
                    ...
                    currentDotView.backgroundColor = [self.delegate lineChartDotsView:self colorForDotAtHorizontalIndex:realIndex atLineIndex:lineIndex];
                }
                ...
                if ([key isKindOfClass:[NSNumber class]])
                {
                    ...
                    if ([dotView isKindOfClass:[JBLineChartDotView class]])
                    {
                        if (weakSelf.selectedLineIndex == lineIndex)
                        {
                            ...
                            dotView.backgroundColor = [self.delegate lineChartDotsView:self selectedColorForDotAtHorizontalIndex:realIndex atLineIndex:lineIndex];
                        }
                        else
                        {
                            NSInteger realIndex = [self.delegate lineChartDotsView:self horizontalRealIndex:horizontalIndex atLineIndex:lineIndex];
                            ...
                            NSAssert([self.delegate respondsToSelector:@selector(lineChartDotsView:colorForDotAtHorizontalIndex:atLineIndex:)], @"JBLineChartDotsView // delegate must implement - (UIColor *)lineChartDotsView:(JBLineChartDotsView *)lineChartDotsView colorForDotAtHorizontalIndex:(NSUInteger)horizontalIndex atLineIndex:(NSUInteger)lineIndex");
                            dotView.backgroundColor = [self.delegate lineChartDotsView:self colorForDotAtHorizontalIndex:realIndex atLineIndex:lineIndex];
                    ...
                    else
                    {
                        ...
                        BOOL hideDotView = [self.delegate lineChartDotsView:self shouldHideDotViewOnSelectionAtHorizontalIndex:realIndex atLineIndex:lineIndex];
                        if (weakSelf.selectedLineIndex == lineIndex)
                        {
